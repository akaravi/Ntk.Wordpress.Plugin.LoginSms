...
//        add_action('login_enqueue_scripts', array(&$this, 'admin_custom_css'));
    ...
    function lwp_add_phonenumber_field($user)
    {
        ...
        ?>
<h3>
    <?php esc_html_e('Personal Information', 'crf'); ?>
</h3>

<table class="form-table">
    <tr>
        <th><label for="phone_number">
                <?php esc_html_e('phone_number', $this->textdomain); ?>
            </label></th>
        <td>
            <input type="text" step="1" id="phone_number" name="phone_number" value="<?php echo esc_attr($phn); ?>"
                class="regular-text" />
...
<div class="wrap">
    <div class="lwp-wrap-left">
        ...
        <h2>
            <?php _e('idehwebLwp Settings', $this->textdomain); ?>
        </h2>
        ...
        <div id="setting-error-settings_updated" class="updated settings-error">
            <p><strong>
                    <?php _e('Settings saved.', $this->textdomain); ?>
                </strong></p>
        </div>
        <?php } ?>
            ...
            idehweb_body.on('click', '.loiuyt',
                function() {
                    ...
                    $.ajax({
                        ...
                        // console.log(arr);
                        $('.creditor .cp').html('<?php _e('Your Credit: ', $this->textdomain) ?>' +
                            ' ' +
                            arr['credit'])
                                ...
                                if (arr && arr.products) {
                                    ...
                                    for (var j = 0; j < arr.products.length; j++) {
                                        $('.chargeAccount').append(
                                            ...
                                            '<a target="_blank" href="' + arr.products[j].buy +
                                            lwp_token + '/' + arr.products[j].ID + '">' +
                                            '<?php _e("Buy", $this->textdomain); ?>' + '</a>' +
                                            '</div>' +
                                            '</div>' +
                                            '</div>'
                                        )
                        ...
                        $.ajax({
                            ...
                            if (msg) {
                                ...
                                if (arr && arr['success']) {
                                    ...
                                    if (arr['err'] && arr['err']['response'] && arr['err']['response'][
                                            'request'
                                        ] && arr['err']['response']['request']['uri'] && arr['err'][
                                            'response'
                                        ]['request']['uri']['host'] === 'localhost') {
                                        $('.lwpmaintextloader').html(
                                            'authentication on localhost not accepted. please use with your domain!'
                                        );
            ...
            function createTutorial() {
                ...
                wrap.prepend('<div class="lwp_modal_overlay"></div>')
                    .prepend('<div class="lwp_modal">' +
                        ...
                        '<ul>' +
                        '<li>' +
                        '<?php _e("1. create a page and name it login or register or what ever", $this->textdomain) ?>' +
                        '</li>' +
                        '<li>' +
                        '<?php _e("2. copy this shortcode <code>[idehweb_lwp]</code> and paste in the page you created at step 1", $this->textdomain) ?>' +
                        '</li>' +
                        '<li>' +
                        '<?php _e("3. now, that is your login page. check your login page with other device or browser that you are not logged in!", $this->textdomain) ?>' +
                        '</li>' +
                        '<li>' +
                        '<?php _e("for more information visit: ", $this->textdomain) ?>' +
                        '<a target="_blank" href="https://idehweb.com/product/login-with-phone-number-in-wordpress/?lang=en">Idehweb</a>' +
                        '</li>' +
                        '</ul>' +
                        '</div>' +
...
<div class="wrap">
    <div id="icon-themes" class="icon32"></div>
    <h2>
        <?php _e('Style settings', $this->textdomain); ?>
    </h2>
    <?php if (isset($_GET['settings-updated']) && $_GET['settings-updated']) {
...
<div class="wrap">
    <div id="icon-themes" class="icon32"></div>
    <h2>
        <?php _e('Localization settings', $this->textdomain); ?>
    </h2>
    <?php if (isset($_GET['settings-updated']) && $_GET['settings-updated']) {
...
<?php
        ...
        ?>
<button type="button" class="button-primary auth i35" value="<?php _e('Authenticate', $this->textdomain); ?>">
    <?php _e('activate sms login', $this->textdomain); ?>
</button>
<button type="button" class="button-primary activate i34" style="display: none"
    value="<?php _e('Activate', $this->textdomain); ?>">
    <?php _e('activate account', $this->textdomain); ?>
</button>
...
<button type="button" class="button-primary authwithwebsite i35"
    value="<?php _e('Authenticate', $this->textdomain); ?>">
    <?php _e('activate sms login', $this->textdomain); ?>
</button>
...
<div class="creditor">
    <button type="button" class="button-primary loiuyt" value="<?php _e('Check credit', $this->textdomain); ?>">
        <?php _e('Check credit', $this->textdomain); ?>
    </button>
    <span class="cp"></span>

    <button type="button" class="button-primary refreshShop" value="<?php _e('Refresh', $this->textdomain); ?>">
        <?php _e('Refresh', $this->textdomain); ?>
    </button>
    <span class="df">
...
//            $localize['firebase_config'] = $this->removePhpComments($options['idehweb_firebase_config']);
    ...
    function idehweb_lwp_metas($vals)
    {
        ...
        if(is_user_logged_in()){
            echo '<p style="text-align:center;" class="lwp user_email"> کاربر عزیز با شماره تماس ' . get_user_meta($user->ID, 'phone_number', true) . 'شما هم اکنون وارد شده اید :) </p>';
            echo '<a class="lwp logout-btn" href="' . wp_logout_url() . '">خروج</a>';
            echo '<a class="lwp after-login-redirect-btn" href="'. get_home_url() .'">بازگشت به خانه</a>';
        }else{
            echo '<p style="text-align:center;" class="lwp user_email">کاربر عزیز، شما وارد نشده اید.</p>';
        }
        return ob_get_clean();
    }
    ...
    function shortcode($atts)
    {
        ...
        if (!$is_user_logged_in) {
            ?>
<a id="show_login" class="show_login" style="display: none"
    data-sticky="<?php echo esc_attr($options['idehweb_position_form']); ?>">
...
<div class="lwp_forms_login <?php echo esc_attr($class); ?>">
    ...
    <form id="lwp_login" class="ajax-auth" action="login" style="<?php echo $cclass; ?>" method="post">
        ...
        <div class="lh1">
            <?php echo isset($localizationـoptions['idehweb_localization_status']) ? esc_html($localizationـoptions['idehweb_localization_title_of_login_form']) : (__('Login / register', $this->textdomain)); ?>
        </div>
        <p class="status"></p>
        <?php wp_nonce_field('ajax-login-nonce', 'security'); ?>
        <div class="lwp-form-box">
            <label class="lwp_labels" for="lwp_username">
                <?php echo __('Phone number', $this->textdomain); ?>
            </label>
            <?php
                            //                    $country_codes = $this->get_country_code_options();
                            ?>
...
//                            $rr=in_array($country["value"],$options['idehweb_country_codes']);
        ...
        <div class="accept_terms_and_conditions">
            ...
            <span class="accept_terms_and_conditions_text">
                <?php echo($options['idehweb_term_and_conditions_text']); ?>
            </span>
        </div>
        <?php } ?>
...
//                    }
    ...
    <form id="lwp_login_email" class="ajax-auth" action="loginemail" style="<?php echo $ecclass; ?>" method="post">
        ...
        <div class="lh1">
            <?php echo isset($localizationـoptions['idehweb_localization_status']) ? esc_html($localizationـoptions['idehweb_localization_title_of_login_form_email']) : (__('Login / register', $this->textdomain)); ?>
        </div>
        <p class="status"></p>
        <?php wp_nonce_field('ajax-login-nonce', 'security'); ?>
        <label class="lwp_labels" for="lwp_email">
            <?php echo __('Your email:', $this->textdomain); ?>
        </label>
        ...
        <?php if ($options['idehweb_enable_accept_terms_and_condition'] == '1') { ?>
        <div class="accept_terms_and_conditions">
            <input class="required lwp_check_box lwp_accept_terms_email" type="checkbox" name="lwp_accept_terms_email"
                checked="checked">
            <span class="accept_terms_and_conditions_text">
                <?php echo($options['idehweb_term_and_conditions_text']); ?>
            </span>
    ...
    <form id="lwp_activate" class="ajax-auth" action="activate" method="post">
        <div class="lh1">
            <?php echo __('Activation', $this->textdomain); ?>
        </div>
        <p class="status"></p>
        <?php wp_nonce_field('ajax-login-nonce', 'security'); ?>
        ...
        <label class="lwp_labels" for="lwp_scode">
            <?php echo __('Security code', $this->textdomain); ?>
        </label>
    ...
    <form id="lwp_update_password" class="ajax-auth" action="update_password" method="post">
        ...
        <div class="lh1">
            <?php echo __('Update password', $this->textdomain); ?>
        </div>
        <p class="status"></p>
        <?php wp_nonce_field('ajax-login-nonce', 'security'); ?>
        <label class="lwp_labels" for="lwp_email">
            <?php echo __('Enter new password:', $this->textdomain); ?>
        </label>
    ...
    <form id="lwp_enter_password" class="ajax-auth" action="enter_password" method="post">
        ...
        <div class="lh1">
            <?php echo __('Enter password', $this->textdomain); ?>
        </div>
        <p class="status"></p>
        <?php wp_nonce_field('ajax-login-nonce', 'security'); ?>
        <label class="lwp_labels" for="lwp_email">
            <?php echo __('Your password:', $this->textdomain); ?>
        </label>
