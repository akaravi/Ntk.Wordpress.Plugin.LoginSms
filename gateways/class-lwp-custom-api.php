...
class LWP_CUSTOM_Api
{
    ...
    public function __construct()
    {
        ...
        if (!isset($options['idehweb_custom_api_header']))
            $options['idehweb_custom_api_header'] = '';
        if (!isset($options['idehweb_custom_api_body']))
            $options['idehweb_custom_api_body'] = '';
        if (!isset($options['idehweb_custom_api_url']))
            $options['idehweb_custom_api_url'] = '';
        if (!isset($options['idehweb_custom_api_method']))
            $options['idehweb_custom_api_method'] = 'GET';
        if (!isset($options['idehweb_custom_api_smstext']))
    ...
    public function lwp_send_sms($phone, $code)
    {
        ...
        $response = false;
        $username = "CHALAKFANAR";
        $password = "123456";
        $to = substr_replace($phone, "0", 0, 2);
        $massage = "چالاک فنر" . "\n" . "کد تایید: " . $code;
        ;
        ...
        $data_to_send = "username=$username&password=$password&to=$to&text=" . urlencode($massage);
        ...
        return (array(
            'success' => true,
            'userMessage' => "پیامک با موفقیت ارسال شد.",
            'devMessage' => "پیامک با موفقیت ارسال شد.",
            'resParam' => array(
                "code" => $code,
                "phone" => $to,
                "massage" => $massage,
