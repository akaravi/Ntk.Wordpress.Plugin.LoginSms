...
jQuery(document).ready(function ($) {
  $(document).on("click", ".lwp_login_overlay, .close", function (e) {
    e.preventDefault();
    $("form#lwp_login, form#lwp_login_email, form#lwp_activate").fadeOut(
      500,
      function () {
  ...
  $("body").on("submit", "form#lwp_login", function (e) {
    console.log("submit form#lwp_login");
    e.preventDefault();
      ...
      success: function (data) {
        if (data.authWithPass) {
          if (!data.updatedPass) {
            ...
            $("p.status", ctrl).text(data.message);
            if (data.success) document.location.href = idehweb_lwp.redirecturl;
          }
        } else {
          $("p.status", ctrl).text(data.message);
          if (data.success) document.location.href = idehweb_lwp.redirecturl;
  ...
  if (jQuery("#lwp_login").length) jQuery("#lwp_login").validate();

  window.lwp_runTimer = function () {
    if (
      idehweb_lwp.timer &&
      (idehweb_lwp.timer == "1" || idehweb_lwp.timer == 1)
    ) {
      var lwp_start = idehweb_lwp.timer_count || 60;
      lwp_refreshIntervalId = setInterval(function () {
        if (lwp_start >= 0) $(".lwp_timer").text(lwp_start--);
        else {
          clearInterval(lwp_refreshIntervalId);
          $(".lwp_didnt_r_c").removeClass("lwp_disable");
          $(".lwp_timer").empty();
